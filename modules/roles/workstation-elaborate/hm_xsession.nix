  ...
  xsession = {
    ...
    windowManager.i3 =
      let
        ...
        cmd_term_tmux =
          "${cmd_term} -t 'tmux-main' -e sh -c 'tmux new -ADs main'";

        cmd_menu =
          "${pkgs.dmenu}/bin/dmenu_run -fn 'Iosevka:size=20' -nb '#000000' -sb '#00fcff' -sf '#000000'";
        ...
        config = {
          ...
          bars = [
...

            {
              ...
              statusCommand =
                "${pkgs.i3status-rust}/bin/i3status-rs ~/.config/i3/i3status-rust_smallscreen.toml";
              extraConfig = ''
                output DP-4
              '';
            }
          ...
          startup = [
            {
              notification = false;
              command = "${pkgs.autorandr}/bin/autorandr -l eiffel";
            }
            {
              notification = false;
              command =
