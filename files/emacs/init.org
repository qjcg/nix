...
** Emacs Basics
   ...
   #+begin_src emacs-lisp
	 ...
	 (add-to-list 'default-frame-alist '(font . "JuliaMono-20")))
       ...
       (custom-file (concat user-emacs-directory "custom.el") "Use a separate file (not init) for customizations") ;; See info(emacs) Saving Customizations.
...
*** Keyboard
    ...
    #+begin_src emacs-lisp
      ...
      ;; Create pretty hydras with a similar API and summon them with the
      ;; same key across different major modes.
	...
	(("M-SPC" . major-mode-hydra)
	 ("<f5>" . my-hydra-toggles/body))

	:config
	(defvar my-hydra-toggles--title
	  (concat (all-the-icons-faicon "toggle-on" :face 'all-the-icons-green) " Toggles"))

	(pretty-hydra-define my-hydra-toggles
	  (:color amaranth :quit-key ("<f5>" "q") :title my-hydra-toggles--title)
	  ("Basic"
	   ...
	   (("f" set-fill-column :title %fill-column)
	    ("v" visual-fill-column-mode :toggle t)
	   ...
	   "UI"
	   (("d" modus-themes-toggle "light/dark theme"))
	   ;; "Coding"
	...
	(which-key-mode))
      ...
      ;; TODO: Install "rigpa"
      ;; Watching for a first non-pre-release on GH, see:
      ;; https://github.com/countvajhula/rigpa/issues/1#issuecomment-743849934
    #+end_src
...
*** Themes
	...
	:ensure
	:config
	(add-to-list 'all-the-icons-mode-icon-alist '(elfeed-search-mode all-the-icons-faicon "rss-square" :face all-the-icons-orange))
	(add-to-list 'all-the-icons-mode-icon-alist '(elfeed-show-mode all-the-icons-faicon "rss-square" :face all-the-icons-orange)))
	...
	(defun my-modus-themes-custom-faces ()
	  "Add my customizations to modus themes"
	  (interactive)
	  (set-face-attribute 'cursor nil :background (modus-themes-color 'magenta-intense)))
...
*** Slides
    ...
    #+begin_src emacs-lisp
      ...
      ;; A package for running demonstrations and presentations from within Emacs.
      ;; https://github.com/howardabrams/demo-it
      (use-package demo-it :ensure)
...
*** General
	...
	:bind (("C-'" . imenu-list-smart-toggle)))
      ...
      ;; TODO: Decide whether this package or imenu-list is
...
*** Go                                                                   :go:
	...
	(defun my-go-run-buffer ()
	  "Run current buffer contents via 'go run'"
	  (interactive)
	  (let ((f (make-temp-file "main_" :suffix ".go" :text )))
	    (write-file f :confirm nil)
	    (shell-command (format "go run %s" f))))
...
*** Behavior Driven Development (BDD)
    ...
    #+begin_src emacs-lisp
      ;; Mode for editing Cucumber (Gherkin) plain text stories
...
** Completion
   ...
   #+begin_src emacs-lisp
	...
	("M-s o"   . helm-occur) ;; search buffer for lines matching pattern
...
*** Web
    ...
    #+name:
    #+begin_src emacs-lisp
      ;; Easy HTTP requests for Emacs Lisp.
      ;; https://github.com/tkf/emacs-request
      (use-package request :ensure)

      ;; Extend shr/eww and other modes with org features and analytics
      ;; capabilities. Also enables archiving web pages to org files with
      ;; shr.
	...
	(defun jg-request-url-as-org (url)
	  "Convenience function to save web content in an org buffer."
	...
	(defun jg-request-url-to-org (url)
	  "Convenience function to save web content in an org file."
	  (interactive "sRequest url: ")
	  (require 'shrface)
	  (require 'request)
	  (request url
...
*** RSS/Atom Feeds
	...
	:mode-hydra
	(elfeed-search-mode
	 (:title " Elfeed Search Mode Commands" :color amaranth :quit-key "q")
	 ("Search"
	  (("1" (elfeed-search-set-filter "+unread +personal") "personal")
	   ("2" (elfeed-search-set-filter "+unread +emacs") "emacs")
	   ("3" (elfeed-search-set-filter "+unread +youtube") "youtube")
	   ("4" (elfeed-search-set-filter "+unread +news") "news")
	   ("5" (elfeed-search-set-filter "+unread +tech") "tech"))
	  "Tag"
	  (("m" my-elfeed-search-toggle-star "star"))))

	(elfeed-show-mode
	 (:title " Elfeed Show Mode Commands")
	 ("Tag"
	  (("m" my-elfeed-show-tag-star "star"))))
	...
	:ensure
	:config
	(setq elfeed-search-print-entry-function #'elfeed-score-print-entry) ;; Print scores in search buffer.
...
*** Audio & Video
	...
	(bongo-mpv-extra-arguments
	 '("--audio-display=no" "--video=no"))
	(bongo-custom-backend-matchers
	 `((mpv local-file "opus" "m4a") ;; Enable mpv to play various file types.
	   (mpv ("http:" "https:") . t)))) ;; Enable mpv for https? URLs.
...
** Web Services
   ...
   #+begin_src emacs-lisp
     ...
     ;; https://github.com/sebasmonia/pepita
     (use-package pepita :ensure)
   #+end_src
** Thin Ice!
   :PROPERTIES:
